...
export default function TripsDashboard({
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "pending" | "accepted" | "active" | "driver_on_the_way" | "completed" | "cancelled"
  >("all");
  ...
  const assignGuestDriverMutation = useMutation({
    mutationFn: async () => {
      if (!detailsTrips?.id) {
        throw new Error(trans?.trips?.errors?.tripIdMissing || "Trip ID is missing");
      }
      ...
      if (hasCarInfo) {
        ...
        if (carInfo.registration_number.trim()) body.car_info.registration_number = carInfo.registration_number.trim();
        if (carInfo.year.trim()) body.car_info.year = carInfo.year.trim();
        if (carInfo.additional_notes.trim()) body.car_info.additional_notes = carInfo.additional_notes.trim();
      }
  ...
  const [selectedNewStatus, setSelectedNewStatus] = useState<"accepted" | "driver_on_the_way" | "active" | "completed" | null>(null);

  const statusTransitionMutation = useMutation({
    mutationFn: async ({ tripId, status }: { tripId: number; status: "accepted" | "driver_on_the_way" | "active" | "completed" }) => {
      return await postData<any>({
    ...
    onSuccess: async (response) => {
      ...
      if (response?.data && typeof response.data === 'object' && 'id' in response.data) {
        ...
        setDetailsTrips(response.data);
      } else if (response?.trip && typeof response.trip === 'object' && 'id' in response.trip) {
      ...
      try {
        await queryClient.refetchQueries({ 
          queryKey: ["my Trips driver", page, filterStatus, searchTerm, dateFrom, dateTo, isPaid, passengerId, driverId, carTypeId, ordering] 
        });
        ...
        if (updatedTrips?.trips && detailsTrips?.id) {
          const updatedTrip = updatedTrips.trips.find((t: any) => t.id === detailsTrips?.id);
          if (updatedTrip) {
            console.log("Updating detailsTrips from refetched list:", updatedTrip);
  ...
  const getNextStatuses = (currentStatus: string): ("accepted" | "driver_on_the_way" | "active" | "completed")[] => {
  ...
  const filteredTrips = !AllTripsLodaing && AllTrips?.trips ? [...AllTrips.trips].sort((a: any, b: any) => {
    ...
    const aPriority = aIsGuestDriver && a.status !== "completed" ? 2 : aIsGuestDriver ? 1 : 0;
    const bPriority = bIsGuestDriver && b.status !== "completed" ? 2 : bIsGuestDriver ? 1 : 0;
  ...
  return (
    <>
      <div className="mt-4">
        <div className="bg-card rounded-xl border-2 border-border p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                <option value="">{trans?.trips?.filterPlaceholders?.all || "All"}</option>
                <option value="true">{trans?.trips?.filterLabels?.paid || "Paid"}</option>
                <option value="false">{trans?.trips?.filterLabels?.unpaid || "Unpaid"}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-3 border-2 bg-background text-foreground border-border rounded-lg focus:outline-none focus:border-primary"
              >
                <option value="-created_at">{trans?.trips?.orderingOptions?.newest || "Newest first"}</option>
                <option value="created_at">{trans?.trips?.orderingOptions?.oldest || "Oldest first"}</option>
                <option value="-trip_date">{trans?.trips?.orderingOptions?.tripNewest || "Latest trip date"}</option>
                <option value="trip_date">{trans?.trips?.orderingOptions?.tripOldest || "Earliest trip date"}</option>
              </select>
            </div>
          </div>
        ...
        {AllTrips && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 mb-6">
            ...
            {/* Revenue Card */}
            <div className="bg-card rounded-xl border-2 border-green-500/60 p-4 transition-all duration-300" dir={locale === "ar" ? "rtl" : "ltr"}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans?.trips?.stats?.Revenue || "Total revenue"}</p>
                  <p className="text-3xl font-bold text-green-500">£{Number(statsAllTime.total_revenue || 0).toFixed(2)}</p>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.statistics?.[0] || "Total Trips"}</p>
                  <p className="text-3xl font-bold text-yellow-600">{statsAllTime.total}</p>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.status?.find((s: any) => s.value === "pending")?.title || "Pending"}</p>
                  <p className="text-3xl font-bold text-orange-600">{statsAllTime.pending}</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.status?.find((s: any) => s.value === "accepted")?.title || "Accepted"}</p>
                  <p className="text-3xl font-bold text-green-600">{statsAllTime.accepted}</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.status?.find((s: any) => s.value === "active")?.title || "Active"}</p>
                  <p className="text-3xl font-bold text-blue-600">{statsAllTime.active}</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.status?.find((s: any) => s.value === "driver_on_the_way")?.title || trans?.trips?.statusLabels?.onTheWay || "On the way"}</p>
                  <p className="text-3xl font-bold text-cyan-600">{statsAllTime.driver_on_the_way}</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.statistics?.[1] || "Completed"}</p>
                  <p className="text-3xl font-bold text-emerald-600">{statsAllTime.completed}</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-1">{trans.statistics?.[3] || "Cancelled"}</p>
                  <p className="text-3xl font-bold text-red-600">{statsAllTime.cancelled}</p>
                </div>
              </div>
            </button>
          </div>
              ...
              <table className="w-full min-w-[820px] text-xs">
                <thead>
                  <tr className="border-b-2 border-border bg-card">
                    {(trans?.trips?.tableHeaders || []).map((trHeader: string) => (
                      <th key={trHeader} className="text-start py-3 px-3 font-bold text-foreground">
                        {trHeader}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  ...
                  <tr><td >
                    ...
                    filteredTrips.map((trip: any, index: any) => {
                      const shouldHighlight = trip.is_guest_driver === true && trip.status !== "completed";
                      return (
                      <tr
                        key={index}
                      ...
                      >
                        <td className="py-3 px-3 font-bold text-foreground max-w-[160px]">
                          <div className="flex items-center gap-2">
                            ...
                            {shouldHighlight && (
                              <span className="px-1.5 py-0.5 bg-yellow-500 text-white text-xs font-bold rounded-full animate-pulse" title="Requires attention">
                                ⚠️
                              </span>
                            )}
                          </div>
                        </td>
                        <td className="py-3 px-3 text-foreground whitespace-nowrap">{trip.id}</td>
                        ...
                        <td className="py-3 px-3 text-foreground whitespace-nowrap">
                          {(trip.distance_miles * 1.61).toFixed()} {trans?.trips?.units?.km || "km"}
                        </td>
                        ...
                        <td className="py-3 px-3 text-center whitespace-nowrap">
                          <div className="flex items-center space-x-2 justify-center">
                            <Button
                              ...
                              title={trans?.trips?.viewDetails || trans?.viewDetails || "View Details"}
                            >
            ...
            {!AllTripsLodaing && (
              <div className="flex items-center justify-between px-6 border-t-2 border-border">
                <div className="text-sm mx-auto text-muted-foreground">
                  ...
                  {AllTrips?.pagination?.num_pages || 1} {trans.pagination.tables}
                </div>
        ...
        <div className="my-5 xl:hidden space-y-4">
          {AllTripsLodaing ? (
                ...
                <div className="flex items-start justify-between mb-3 pb-3 border-b-2 border-border">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-bold text-lg text-foreground">#{trip.id}</h3>
                      {shouldHighlight && (
                        <span className="px-2 py-0.5 bg-yellow-500 text-white text-xs font-bold rounded-full animate-pulse" title="Requires attention">
                          ⚠️
                        </span>
                      )}
                    </div>
                  ...
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(trip.status)}`}>
                    {trans?.status?.find((s: any) => s.value === trip.status)?.title || trip.status}
                  </span>
                </div>

                <div className="space-y-2 mb-4">
                  <div className="flex items-center justify-between py-2 border-b border-border">
                    <span className="text-sm text-muted-foreground font-semibold">{trans?.trips?.mobileLabels?.pickup || "Pickup"}</span>
                    <span className="text-sm text-foreground font-bold max-w-[60%] truncate" title={trip.pickup_str}>{trip.pickup_str}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-border">
                    <span className="text-sm text-muted-foreground font-semibold">{trans?.trips?.mobileLabels?.dropoff || "Dropoff"}</span>
                    <span className="text-sm text-foreground font-bold max-w-[60%] truncate" title={trip.dropoff_str}>{trip.dropoff_str}</span>
                  </div>
                  <div className="flex items-center justify-between py-2">
                    <span className="text-sm text-muted-foreground font-semibold">{trans?.trips?.mobileLabels?.date || "Date"}</span>
                    <span className="text-sm text-foreground font-bold">{formatDate(new Date(trip.trip_date), locale)} {trip.trip_time?.slice(0,5)}</span>
                  </div>
                </div>

                <Button onClick={() => { setDetailsTrips(trip); setOpenDetails(true); }} className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-foreground font-bold shadow-lg shadow-yellow-500/20">
                  <BsEyeFill className="mr-2" /> {trans?.trips?.viewDetails || trans?.viewDetails || "View Details"}
                </Button>
              </div>
              );
            })
            ...
            <div className="bg-card rounded-xl shadow-lg border-2 border-border p-8 text-center">
              <p className="text-muted-foreground font-medium">{trans?.trips?.noTrips || "No trips found"}</p>
            </div>
          )}
        </div>
      </div>
          ...
          >
            ...
            <div className="sticky top-0 bg-gradient-to-r from-card to-background px-4 xl:px-6 py-4 lg:py-5 border-b-2 border-border rounded-t-2xl z-10">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                >
                  {trans?.status?.find((s: any) => s.value === detailsTrips?.status)?.title || detailsTrips?.status}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 md:p-6 space-y-5 md:space-y-6">
              <div className="flex max-md:flex-col gap-4">
                <div className="md:w-1/2 bg-muted border-2 border-yellow-500/50 rounded-xl p-4 md:p-5 overflow-y-auto on-scrollbar">
                  ...
                  <TripDetails detailsTrips={detailsTrips} trans={trans} locale={locale} variant="dashboard"/>
                </div>
            ...
            <div className="sticky bottom-0 bg-gradient-to-t from-card to-background px-4 md:px-6 py-4 md:py-5 border-t-2 border-border rounded-b-2xl">
              ...
              {detailsTrips?.is_guest_driver === true && detailsTrips?.status !== "completed" && getNextStatuses(detailsTrips?.status || "").length > 0 && (
                <div className="mb-4 p-4 bg-yellow-500/10 border-2 border-yellow-500/50 rounded-xl">
                  <p className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                    <span>⚠️</span>
                    {trans?.trips?.statusTransitionTitle || "Update Trip Status"}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {getNextStatuses(detailsTrips?.status || "").map((nextStatus) => (
                      <Button
                        key={nextStatus}
                        type="button"
                        onClick={() => {
                      ...
                      >
                        {trans?.trips?.statusTransitions?.[nextStatus] || nextStatus.replace(/_/g, " ")}
                      </Button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {(
                  ...
                  >
                    {trans?.trips?.assignToNewDriver || trans?.trips?.assignGuestDriver || "Assign to new driver"}
                  </Button>
                )}
              </div>
            </div>
      ...
      >
        <div className="p-4 sm:p-6 w-full">
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              if (!guestDriverForm.name.trim() || !guestDriverForm.phone.trim()) {
                toast.error(
          ...
          >
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">
                {trans?.trips?.assignGuestNameLabel || trans?.trips?.guestNameLabel || "Guest driver name"}
              ...
              <input
                ...
                placeholder={
                  trans?.trips?.assignGuestNamePlaceholder || trans?.trips?.guestNamePlaceholder || "John Doe"
                }
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">
                {trans?.trips?.assignGuestPhoneLabel || trans?.trips?.guestPhoneLabel || "Guest driver phone"}
                <span className="text-red-500"> *</span>
              </label>
              <div
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">
                {trans?.trips?.assignGuestCompanyLabel || trans?.trips?.guestCompanyLabel || "Company (optional)"}
              </label>
              <input
                type="text"
                ...
                placeholder={
                  trans?.trips?.assignGuestCompanyPlaceholder || trans?.trips?.guestCompanyPlaceholder || "Uber"
                }
              />
            </div>
            ...
            <div className="pt-4 border-t border-border">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">
                    {trans?.trips?.carRegistrationLabel || "Registration Number"}
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full p-3 border-2 border-border rounded-lg bg-background text-foreground focus:outline-none focus:border-primary transition-colors"
                    placeholder={trans?.trips?.carRegistrationPlaceholder || "ABC123"}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-foreground mb-1">
                    {trans?.trips?.carAdditionalNotesLabel || "Additional Notes (Optional)"}
                  </label>
                  <textarea
                    ...
                    placeholder={trans?.trips?.carAdditionalNotesPlaceholder || "Clean interior"}
                  />
                </div>
              </div>
            </div>
      ...
      <GlobalModal
        ...
        maxWidth="md"
      >
        <div className={`p-4 md:p-5 w-full ${locale === "ar" ? "text-right" : "text-left"}`} dir={locale === "ar" ? "rtl" : "ltr"}>
          <h2 className={`text-lg font-bold text-foreground mb-2 ${locale === "ar" ? "text-right" : "text-left"}`}>
            {trans?.trips?.statusTransitionConfirmTitle || "Confirm Status Change"}
          </h2>
          <p className={`text-sm text-muted-foreground mb-3 ${locale === "ar" ? "text-right" : "text-left"}`}>
            {trans?.trips?.statusTransitionConfirmMessage || "Are you sure you want to change the trip status?"}
          </p>
          {selectedNewStatus && (
            <div className={`mb-3 p-3 bg-muted/50 rounded-lg border border-border ${locale === "ar" ? "text-right" : "text-left"}`}>
              <div className={`flex items-center justify-between gap-2 mb-2 ${locale === "ar" ? "flex-row-reverse" : ""}`}>
                <span className="text-xs text-muted-foreground">
                  {trans?.trips?.currentStatus || "Current Status"}:
                </span>
                <span className={`text-xs font-semibold px-2 py-1 rounded ${getStatusColor(detailsTrips?.status || "")}`}>
                  {trans?.status?.find((s: any) => s.value === detailsTrips?.status)?.title || detailsTrips?.status}
                </span>
              </div>
              <div className={`flex items-center justify-center my-1 ${locale === "ar" ? "rotate-180" : ""}`}>
                <svg className="w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </div>
              <div className={`flex items-center justify-between gap-2 ${locale === "ar" ? "flex-row-reverse" : ""}`}>
                <span className="text-xs text-muted-foreground">
                ...
                <span className="text-xs font-semibold text-yellow-600 bg-yellow-500/20 px-2 py-1 rounded border border-yellow-500/30">
                  {trans?.trips?.statusTransitions?.[selectedNewStatus] || selectedNewStatus.replace(/_/g, " ")}
                </span>
              </div>
            </div>
          )}
          <div className={`flex items-center gap-2 mt-4 ${locale === "ar" ? "flex-row-reverse" : ""}`}>
            <Button
              type="button"
              variant="secondary"
              onClick={() => {
            ...
            <Button
              ...
              disabled={statusTransitionMutation.isPending || !selectedNewStatus || !detailsTrips?.id}
