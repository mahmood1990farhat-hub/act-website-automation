...
export default function ModernConfirmFlightDetails({
  ...
  const pickup = data.routePoints.find((r) => r.type === "pickup")?.point?.description || "-";
  const stops = data.routePoints.filter((r) => r.type === "stop").map((r) => r.point?.description || "-");
  const dropoff = data.routePoints.find((r) => r.type === "dropoff")?.point?.description || "-";
    ...
    <div className={`w-full max-w-7xl mx-auto`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-white/80 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto">
          {isRTL ? "تأكد من تفاصيل رحلتك قبل المتابعة" : "Review your trip details before proceeding"}
        </p>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          <ChevronLeft className={`w-4 h-4 sm:w-5 sm:h-5 ${isRTL ? 'rotate-180 ml-2' : 'mr-2'}`} />
          {texts.back}
        </Button>
      </div>
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center gap-2 text-white/60 mb-1">
                      ...
                      <span className="text-xs sm:text-sm font-medium">{texts.pickup}</span>
                    </div>
                  ...
                  {stops.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 text-white/60 mb-1">
                        ...
                        <span className="text-xs sm:text-sm font-medium">{texts.stops}</span>
                      </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 text-white/60 mb-1">
                      <MapPin className="w-4 h-4" />
                      <span className="text-xs sm:text-sm font-medium">{texts.dropoff}</span>
                    </div>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center gap-2 text-white/60 mb-1">
                      ...
                      <span className="text-xs sm:text-sm font-medium">{texts.date}</span>
                    </div>
                    <p className="text-white text-sm">{new Date(data.date).toLocaleDateString(locale, { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>

                  <div>
                    <div className="flex items-center gap-2 text-white/60 mb-1">
                      <Clock className="w-4 h-4" />
                      <span className="text-xs sm:text-sm font-medium">{texts.time}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 text-white/60 mb-1">
                      ...
                      <span className="text-xs sm:text-sm font-medium">{texts.distance}</span>
                    </div>
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-white/60 text-xs sm:text-sm">{texts.passengers}</p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="text-white font-bold text-lg sm:text-xl">{data.smallSuitcase}</p>
                  <p className="text-white/60 text-xs sm:text-sm">{texts.smallLuggage}</p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="text-white font-bold text-lg sm:text-xl">{data.largeSuitcase}</p>
                  <p className="text-white/60 text-xs sm:text-sm">{texts.largeLuggage}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-white/60">
                    <span className="text-sm">{data.numberOfPassengers} {texts.passengers}</span>
                    <Badge variant="outline" className="bg-[#ffd100]/20 border-[#ffd100]/50 text-[#ffd100] text-xs">
          ...
          <Card className="bg-white/10 backdrop-blur-xl border border-white/20 shadow-xl">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                ...
                {airport_vat > 0 && (
                  <div className={`flex justify-between items-center`}>
                    ...
                    <span className="text-white font-semibold">£{airport_vat.toFixed(2)}</span>
                  </div>
                )}
                ...
                {vat > 0 && (
                  <div className={`flex justify-between items-center`}>
                    ...
                    <span className="text-white font-semibold">£{vat.toFixed(2)}</span>
                  </div>
                )}
              </div>
              ...
              <div className={`flex justify-between items-center`}>
                <span className="text-white font-bold text-lg sm:text-xl">{texts.totalCost}</span>
                <Badge variant="secondary" className="bg-[#ffd100] text-[#2D2E2E] font-bold text-lg sm:text-xl px-3 py-2">
                  £{totalCost?.toFixed(2)}
                </Badge>
              </div>
            </CardContent>
