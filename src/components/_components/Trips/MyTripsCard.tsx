...
export default function MyTripsCard({ data, trans, token, locale, tripCardTrans }: typeProps) {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {/* Status Badge */}
          <div className={`px-3 py-1.5 rounded-lg border font-semibold text-xs md:text-sm ${getStatusColor(data.status)}`}>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4">
          ...
          <div className="bg-background/30 rounded-lg p-3 border border-muted/10">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-gray-400 text-xs font-medium">{tripCardTrans?.distance || "Distance"}</span>
            </div>
          ...
          <div className="bg-background/30 rounded-lg p-3 border border-muted/10">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-gray-400 text-xs font-medium">{tripCardTrans?.duration || "Duration"}</span>
            </div>
            <p className="text-white font-semibold text-sm md:text-base">
        ...
        <div className="bg-background/30 rounded-lg p-3 border border-muted/10 mb-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-gray-400 text-xs font-medium">{tripCardTrans?.destination || "Destination"}</span>
          </div>
        ...
        <div className="flex flex-col gap-3 pt-3 border-t border-muted/10">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-gray-400 text-xs">{tripCardTrans?.gbp || "GBP"}</span>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 w-full">
            ...
            >
              ...
              <span className="max-sm:hidden">{tripCardTrans?.seeDetails || "See Details"}</span>
              <span className="sm:hidden">{tripCardTrans?.details || "Details"}</span>
            </button>
            ...
            {data.status === "pending" && (
              ...
              >
                ...
                <span className="max-sm:hidden">{tripCardTrans?.cancel || "Cancel"}</span>
              </button>
            )}
          </div>
        </div>
            ...
            <div className="bg-gradient-to-r from-primary/10 to-accent/10 border-b border-muted/20 px-6 py-5">
              <h2 className="text-2xl md:text-3xl font-bold text-center">
                {tripCardTrans?.tripDetails ? (
                  <>
                    <span className="text-primary">{tripCardTrans.tripDetails.split(" ")[0]}</span>{" "}
                    <span className="text-white">{tripCardTrans.tripDetails.split(" ").slice(1).join(" ")}</span>
                  </>
                ) : (
                  <>
              ...
              <div className="flex flex-col lg:flex-row gap-6 h-full">
                ...
                <div className="lg:w-1/2 space-y-4">
                  ...
                  <div className="flex justify-center lg:justify-start">
                    <div className={`px-4 py-2 rounded-lg border font-semibold ${getStatusColor(data.status)}`}>
                      {tripCardTrans?.status || "Status:"} {tripCardTrans?.statusLabels?.[data.status] || data.status}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="bg-background/30 rounded-lg p-4 border border-muted/10">
                      <div className="flex items-center gap-2 mb-2">
                        <FaCalendarAlt className="text-primary" />
                        <span className="text-gray-400 text-sm font-medium">{tripCardTrans?.date || "Date"}</span>
                      </div>
                      <p className="text-white font-semibold">{data.trip_date}</p>
                    </div>
                    ...
                    <div className="bg-background/30 rounded-lg p-4 border border-muted/10">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-gray-400 text-sm font-medium">{tripCardTrans?.distance || "Distance"}</span>
                      </div>
                     ...
                     <div className="bg-background/30 rounded-lg p-4 border border-muted/10">
                       <div className="flex items-center gap-2 mb-2">
                         ...
                         <span className="text-gray-400 text-sm font-medium">{tripCardTrans?.duration || "Duration"}</span>
                       </div>
                       <p className="text-white font-semibold">{data.expected_trip_duration_minutes || "0"}</p>
                     </div>
                     ...
                     <div className="bg-background/30 rounded-lg p-4 border border-muted/10">
                       <div className="flex items-center gap-2 mb-2">
                         ...
                         <span className="text-gray-400 text-sm font-medium">{tripCardTrans?.cost || "Cost"}</span>
                       </div>
                       <p className="text-primary font-bold text-xl">£{data.cost}</p>
                     </div>
                   </div>
                   ...
                   <div className="bg-background/30 rounded-lg p-4 border border-muted/10 sm:col-span-2">
                     <div className="flex items-center gap-2 mb-2">
                       ...
                       <span className="text-gray-400 text-sm font-medium">{tripCardTrans?.destination || "Destination"}</span>
                     </div>
                    <p className="text-white font-semibold">{data.destination || "London"}</p>
                  </div>
                </div>
