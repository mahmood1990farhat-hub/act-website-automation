...
def _get_demo_password() -> str | None:
    return getattr(settings, "DEMO_PASSWORD", None) or os.environ.get("DEMO_PASSWORD")
...
def seed_demo_users(stdout, style, dry_run: bool = False) -> dict:
    ...
    if not demo_password:
        stdout.write(
            style.WARNING(
                "DEMO_PASSWORD is not configured; skipping demo user creation."
            )
        )
        return refs
...
def run_seed(
    ...
    *,
    ...
    if not dry_run:
        password_message = (
            "\nDemo users were created with the configured DEMO_PASSWORD.\n"
            if _get_demo_password()
            else "\nDemo users were not created because DEMO_PASSWORD is not configured.\n"
        )
        stdout.write(
            style.SUCCESS(
                password_message
