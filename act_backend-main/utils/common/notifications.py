...
def notify_all_drivers(title_en, title_ar, desc_en, desc_ar, locale='en', mobile_url=None, web_url=None, trip_id=None, notification_type=None):
    """
    Notify active drivers with device tokens about a new trip request.
    When trip_id is provided, only drivers whose vehicle type matches the trip's car_type are notified.
    ...
    Args:
        ...
        mobile_url: Optional mobile app deep link
        web_url: Optional web URL
        trip_id: Optional trip ID for deep linking and vehicle type filtering
    ...
    if trip_id:
        try:
            trip = Trip.objects.select_related('car_type').get(pk=trip_id)
            vehicle_type = trip.car_type
        except Trip.DoesNotExist:
            logger.warning(f"[NOTIFY_ALL_DRIVERS] Trip {trip_id} not found")
            return {
                'success_count': 0,
                'failure_count': 0,
                'total_drivers': 0
    ...
    if vehicle_type:
        ...
        logger.info(
            f"[NOTIFY_ALL_DRIVERS] Filtering drivers by vehicle type: {vehicle_type} (trip_id={trip_id})"
        )
    elif trip_id:
        logger.warning(
